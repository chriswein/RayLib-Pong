...
public:
	Game(void (*callback)(void*)) : Level(callback) {}
	virtual void init()
	{
		this->game_elements =
	...
	virtual void update()
	{
		...
		if (!this->ball1->living)
		{
			...
			// this->init();
			((void(*)(void*))this->callback)(nullptr);
		}
	}
	virtual void draw()
	{
...
class Title : public Level {
	public:
	double start;
	Title(void (*callback)(void*)) : Level(callback) {}
	virtual void update(){
		if(GetTime() > start+5){
			//Game g(this->callback);
